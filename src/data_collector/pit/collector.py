...
def _convert_numeric_preserve_non_numeric(series: pd.Series) -> pd.Series:
...
class PitCollector(BaseCollector):
    ...
    def get_performance_express_report_df(code: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        report_df["value"] = _convert_numeric_preserve_non_numeric(report_df["value"])
    ...
    def get_profit_df(code: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        profit_df["value"] = _convert_numeric_preserve_non_numeric(profit_df["value"])
        return profit_df
    ...
    def get_forecast_report_df(code: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            forecast_df[numeric_fields] = forecast_df[numeric_fields].apply(
                _convert_numeric_preserve_non_numeric
            )
        except KeyError:
            return pd.DataFrame()
    ...
    def get_growth_df(code: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        growth_df["value"] = _convert_numeric_preserve_non_numeric(growth_df["value"])
        return growth_df
