...
<!--
> 若你需要严格复现下表结果，请使用 **v1** 数据：`python src/scripts/get_data.py qlib_data --target_dir ~/.qlib/qlib_data/cn_data --region cn --version v1`
>
...
1. 为你的模型创建一个子目录（参考 `src/examples/benchmarks/TCTS` 的目录组织方式）。
2. 子目录中建议包含：
...
4. 更新本页面的 Benchmark 表格：表格中的每个指标建议基于 **20 个随机种子** 的均值±标准差。你也可以使用 `src/examples/run_all_model.py` 做批量运行与汇总。
