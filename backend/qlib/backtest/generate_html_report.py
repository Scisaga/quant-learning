...
- 不指定 recorder：自动选择该 experiment 下最新的 recorder：
  `python backend/qlib/backtest/generate_html_report.py --exp-name tutorial_exp`
...
# 确保 backend/qlib 根目录在 sys.path 中，便于通过 module_path 加载自定义策略（如 backtest.strategy.StabilizedHoldingStrategy）。
_SRC_ROOT = Path(__file__).resolve().parents[1]  # backend/qlib
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # 训练脚本（如 backend/qlib/train/train_lgb_alpha158_pit.py）通常会打印 recorder_id。
