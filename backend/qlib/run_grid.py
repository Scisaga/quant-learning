...
- 这里用 ThreadPoolExecutor 并行跑“外部子进程”：
  - `python backend/qlib/train/train_lgb_alpha158_pit.py ...`
  - `python backend/qlib/backtest/generate_html_report.py ...`
...
def _run_one_job(
    *,
    ...
    train_cmd = [
        python_exe,
        "backend/qlib/train/train_lgb_alpha158_pit.py",
        "--provider-uri",
        provider_uri,
        "--market",
        job.market,
    ...
    report_cmd = [
        python_exe,
        "backend/qlib/backtest/generate_html_report.py",
        "--provider-uri",
        provider_uri,
        "--exp-name",
        job.exp_name,
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # 文件在 backend/qlib/ 下，repo_root 为项目根目录。
