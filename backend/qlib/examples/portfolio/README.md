...
在 `backend/qlib/examples/benchmarks/` 中，我们提供了多种 **alpha** 模型用于预测股票收益，并使用一个简单的规则策略 `TopkDropoutStrategy` 来评估投资表现。
...
```bash
python backend/qlib/examples/portfolio/prepare_riskdata.py
```
...
```bash
qrun backend/qlib/examples/portfolio/config_enhanced_indexing.yaml
```

该配置与 `backend/qlib/examples/benchmarks/LightGBM/workflow_config_lightgbm_Alpha158.yaml` 的主要差异集中在 strategy 部分（用优化策略替换简单 TopK 策略）。
