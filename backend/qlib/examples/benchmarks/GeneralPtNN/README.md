...
- `workflow_config_gru.yaml`：对齐历史 GRU 基线结果（参考 `backend/qlib/examples/benchmarks/README.md` 中的 GRU 行）。
- `workflow_config_gru2mlp.yaml`：展示如何在 **最小改动** 下把时序配置转换为表格配置（关键是切换 net 与 dataset 类）。
- `workflow_config_mlp.yaml`：展示与 MLP 类似的功能（同样可参考 `backend/qlib/examples/benchmarks/README.md` 的 MLP 行）。
...
```bash
cd backend/qlib/examples/benchmarks/GeneralPtNN
